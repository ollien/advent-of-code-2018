...
func parseInput(rawRegex string) (*node, []*node, error) {
	startCursor := cursor{
		n:   newNode(),
		row: 0,
		col: 0,
	}
	head, roomGrid, _, err := makeGraph(rawRegex, startCursor, nil)
	if err != nil {
		return nil, nil, err
	}
	head.distance = 0
	return head, roomGrid.flatten(), err
}
...
func main() {
	...
	head, nodes, err := parseInput(string(rawRegex))
	if err != nil {
		panic(err)
	}
